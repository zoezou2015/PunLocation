...
class LM_LSTM_CRF(nn.Module):
    ...
    def __init__(self, tagset_size, charset_size, char_emb_dim, char_rnn_dim, char_rnn_layers, vocab_size,
                 lm_vocab_size, word_emb_dim, word_rnn_dim, word_rnn_layers, dropout, config, highway_layers=1):
        """
        :param tagset_size: number of tags
        ...
        :param lm_vocab_size: vocabulary size of language model (in-corpus words subject to word frequency threshold)
        ...
        self.use_pos_mask = config.use_pos_mask
        ...
        if self.use_pos_mask:
            lstm_input_dim = self.word_emb_dim + self.char_rnn_dim * 2 + 1
        else:
            lstm_input_dim = self.word_emb_dim + self.char_rnn_dim * 2
        self.word_blstm = nn.LSTM(lstm_input_dim, self.word_rnn_dim // 2,
    ...
    def forward(self, cmaps_f, cmaps_b, cmarkers_f, cmarkers_b, wmaps, tmaps, wmap_lengths, cmap_lengths, pos_mask):
        """
        Forward propagation.
...
class ViterbiLoss(nn.Module):
    ...
    def __init__(self, tag_map, config):
        """
        ...
        self.pad_tag = tag_map['<pad>']
        self.penalty = 0
        self.device = config.device
        if torch.cuda.is_available():
            # self.pun_loc = tag_map[1]
            # self.after_pun_loc = tag_map[2]
            # self.before_pun_loc = tag_map[0]
            pass
        else:
            # self.pun_loc = tag_map[1]
            # self.after_pun_loc = tag_map['2']
